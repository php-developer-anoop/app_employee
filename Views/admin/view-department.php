        ...
        <div class="container-fluid">
            <div class="card ">
                <div class="card-header">
                    <a href="<?= base_url(ADMINPATH . 'add-department') ?>" class="btn btn-success m-auto float-right">Add Department</a>
                </div>
                <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'department-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'department-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "", "name": "Department Detail", "title": "Department Detail","render":department },
...
return data;
    ...
    function action_render(data, type, row, meta) {
        ...
        if (type === 'display') {
            ...
            var onclick = "remove('" + row.id + "','dt_department')";
            output = '<a href="<?= base_url(ADMINPATH . "add-department?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white" title="Edit Department"><i class="fa fa-edit"></i></a> ';
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_department')`;
