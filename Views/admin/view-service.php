        ...
        <div class="container-fluid">
            <div class="card ">
                <div class="card-header">
                    <a href="<?= base_url(ADMINPATH . 'add-service') ?>" class="btn btn-success m-auto float-right">Add Service</a>
                </div>
                <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'service-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'service-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "service_name", "name": "Service Name", "title": "Service Name"},
...
return data;
    ...
    function action_render(data, type, row, meta) {
        ...
        if (type === 'display') {
            ...
            var onclick = "remove('" + row.id + "','dt_scope_of_services_list')";
            output = '<a href="<?= base_url(ADMINPATH . "add-service?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white" title="Edit Disease"><i class="fa fa-edit"></i></a> ';
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_scope_of_services_list')`;
