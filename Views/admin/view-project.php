        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <a href="<?= base_url(ADMINPATH . 'add-project') ?>" class="btn btn-success m-auto float-right">Add Project</a>
                </div>
                <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'project-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'project-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
...
return data;
    ...
    function action_render(data, type, row, meta) {
        ...
        if (type === 'display') {
            ...
            var onclick = "remove('" + row.id + "','dt_project_master')";
            output = '<a href="<?= base_url(ADMINPATH . "add-project?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white" title="Edit Program"><i class="fa fa-edit"></i></a> ';
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_project_master')`;
