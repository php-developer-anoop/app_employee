        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <a href="<?= base_url(ADMINPATH . 'add-program') ?>" class="btn btn-success m-auto float-right">Add Program</a>
                </div>
                <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'program-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'program-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "program_name", "name": "Program Name", "title": "Program Name" },
            { data: "", "name": "Validity", "title": "Validity","render":validity },
...
var validity = ( data, type, row, meta )=>{
  var data = '';
...
return data;
    ...
    function action_render(data, type, row, meta) {
        ...
        if (type === 'display') {
            ...
            var onclick = "remove('" + row.id + "','dt_program_master')";
            output = '<a href="<?= base_url(ADMINPATH . "add-program?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white" title="Edit Program"><i class="fa fa-edit"></i></a> ';
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_program_master')`;
