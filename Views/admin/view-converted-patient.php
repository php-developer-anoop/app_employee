        ...
        <div class="card-header">
          <div class="row justify-content-center">
            ...
            <div class="form-group col-lg-2 mt-4 pt-2">
              ...
              <a href="<?=base_url(ADMINPATH.'case-history')?>" class="btn btn-success">Reset</a>
            </div>
...
<script>
  ...
  $.ajax({
    url: '<?= base_url(ADMINPATH .'converted-patient-data');?>?'+qparam,
    type : "POST",
    data: {
      'is_count': 'yes',
      'start': 0,
...
function loadAllRecordsData(qparam) {
  ...
  $('#responseData').DataTable({
    ...
    "serverSide": true,
    "ajax": {
      "url": '<?= base_url(ADMINPATH .'converted-patient-data');?>'+newQueryParam,
      "type": 'POST',
      dataSrc: (res) => {
        return res.data
      }
...
function validatefilter(){
   ...
   if(from_date==""){
    ...
    window.location.href="<?=base_url(ADMINPATH.'case-history?from_date=')?>"+from_date+"&to_date="+to_date+"";
   }
  } 
