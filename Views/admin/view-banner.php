...
<script>
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            { data: "app_type", "name": "App Type", "title": "App Type"},
            { data: "", "name": "Banner", "title": "Banner","render":banner },
...
function banner(data, type, row, meta){
  let output = '';
  let banner= row.banner!=null?row.banner:"";
  if(type === 'display'){
  output += '<img height="50" src="'+banner+'" />';
  ...
  return output;
    ...
    function action_render(data, type, row, meta) {
        ...
        if (type === 'display') {
            ...
            var onclick = "remove('" + row.id + "','dt_banner_master')";
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_banner_master')`;
