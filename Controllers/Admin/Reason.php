...
class Reason extends BaseController {
    ...
    public function __construct() {
        ...
        $this->table = "dt_reason_master";
    }
    function index() {
        $data = [];
        $data["title"] = "Reason Master";
        adminview('view-reason', $data);
    }
    function add_reason() {
        $id = !empty($this->request->getVar('id')) ? $this->request->getVar('id') : '';
        $data = [];
        $data["title"] = !empty($id)?"Edit Reason":"Add Reason";
        ...
        $data['reason'] = !empty($savedData['reason']) ? $savedData['reason'] : '';
        $data['status'] = !empty($savedData['status']) ? $savedData['status'] : 'Active';
        adminview('add-reason', $data);
    }
    ...
    public function getRecords() {
        ...
        if (!empty($get["search"]["value"])) {
            ...
            $where[" reason LIKE '%" . $searchString . "%' OR type LIKE '%" . $searchString . "%'"] = null;
            $limit = 100;
            $start = 0;
        }
