...
class Category extends BaseController {
    ...
    public function __construct() {
        ...
        $this->table = "dt_category_list";
    }
    function index() {
        $data = [];
        $data["title"] = "Category Master";
        adminview('view-category', $data);
    }
    function add_category() {
        $id = !empty($this->request->getVar('id')) ? $this->request->getVar('id') : '';
        $data = [];
        $data["title"] = !empty($id)?"Edit Category":"Add Category";
    ...
    public function save_category() {
        ...
        if($duplicate && empty($id)){
            ...
            return redirect()->to(base_url(ADMINPATH . 'category-list')); 
        }
