...
class Disease extends BaseController {
    ...
    public function __construct() {
        ...
        $this->table = "dt_disease_list";
    }
    function index() {
        $data = [];
        $data["title"] = "Speciality Master";
        adminview('view-disease', $data);
    }
    function add_disease() {
        $id = !empty($this->request->getVar('id')) ? $this->request->getVar('id') : '';
        $data = [];
        $data["title"] = !empty($id)?"Edit Speciality":"Add Speciality";
        $savedData = $this->c_model->getSingle($this->table, '*', ['id' => $id]);
        $data['id'] = !empty($savedData['id']) ? $savedData['id'] : $id;
        $data['disease_name'] = !empty($savedData['disease_name']) ? $savedData['disease_name'] : '';
        $data['status'] = !empty($savedData['status']) ? $savedData['status'] : 'Active';
        adminview('add-disease', $data);
    }
    ...
    public function getRecords() {
        ...
        if (!empty($get["search"]["value"])) {
            ...
            $where[" disease_name LIKE '%" . $searchString . "%'"] = null;
            $limit = 100;
            $start = 0;
        }
