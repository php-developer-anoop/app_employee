...
if (!function_exists('sendEmail')) {
    function sendEmail($to, $username = false,$password=false){
        ...
        //$email->clear(true);
        $email->setTo($to);
        $email->setFrom('info@emailnotify.in', 'Employee App');
        $email->setSubject('Employee App || Login Credentials');
        $email->setMessage('<html><head>
        <title>Employee App || Login Credentials</title></head>
        <body>
        <h3>Dear User</h3>
        <h3>Greetings From Employee App !</h3>
        ...
        <a href="mailto:info@employee.nshops.in">info@employee.nshops.in</a> </p>
        <h3>Regards :-  Employee App.</h3>
        </body>
        </html>');
...
if (!function_exists('sendEmailForgotPassword')) {
    function sendEmailForgotPassword($to, $username = false,$password=false){
        ...
        $email->setTo($to);
        $email->setFrom('info@emailnotify.in', 'Employee App');
        $email->setSubject('Employee App || New Password');
        $email->setMessage('<html><head>
        <title>Employee App || New Password</title></head>
        <body>
        <h3>Dear User</h3>
        <h3>Greetings From Employee App !</h3>
